...
const illuminaStelle = function () {
    ...
    stelle.forEach((element, index) => { // per ogni stella aggiungiamo un attributo valore (la prima avra' 1, la seconda 2, ecc.)
        let value = index + 1; 
        ...
        element.addEventListener('click', function () {
            stelle.forEach((element, index) => { 
                if (index < value) { //se la stella cliccata ha un valore (value) minore delle stelle accese 
                    ...
                    element.classList.remove('illuminaStelle'); //spegli tutte le stelle con valore maggiore
                }
            });
        });
    });
